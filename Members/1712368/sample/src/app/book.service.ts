...
export class BookService {
  ...
  getBooks(category: string) {
    this.http.get<{ message: string, books: any }>("http://localhost:25987/api/books/" + category).pipe(map(bookData => {
  ...
  getCategories() {
    return this.http.get<{ message: string, categories: [] }>("http://localhost:25987/api/categories");
  }
  getBook(bookId: string) {
    return this.http.get<{
      ...
      totalPages: number,
      introduction: string
    }>("http://localhost:25987/api/books/book/" + bookId);
  }
  ...
  addBook(image: File, name: string, author: string, category: string, price: number, publisher: string, publicationDate: string,
    ...
    this.http.post<{ message: string, book: Book }>("http://localhost:25987/api/books", bookData).subscribe(responseData => {
